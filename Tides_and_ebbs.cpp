...
/*Earth.pos.x += Earth.v.x;
Earth.pos.y += Earth.v.y;

Moon.pos.x += Moon.v.x;
Moon.pos.y += Moon.v.y;

double dist_x = Earth.pos.x - Moon.pos.x;
double dist_y = Earth.pos.y - Moon.pos.y;
double dist = sqrt(dist_x * dist_x + dist_y * dist_y);

double sin = dist_x / dist;
double cos = dist_y / dist;

Earth.v.x -= sqrt((G * Moon.m) / dist) * sin / pow(10, 17);
Earth.v.y -= sqrt((G * Moon.m) / dist) * cos / pow(10, 17);

Moon.v.x += sqrt((G * Earth.m) / dist) * sin / pow(10, 17);
Moon.v.y += sqrt((G * Earth.m) / dist) * cos / pow(10, 17);


if (dist < Moon.r + Earth.r) {
double Vn1 = Earth.v.x * sin + Earth.v.y * cos;
double Vn2 = Moon.v.x * sin + Moon.v.y * cos;
double dt = (Earth.r + Moon.r - dist) / (Vn1 - Vn2);
if (dt > 0.6) dt = 0.6;
if (dt < -0.6) dt = -0.6;
Moon.pos.x -= Moon.v.x * dt;
Moon.pos.y -= Moon.v.y * dt;
Earth.pos.x += Earth.v.x * dt;
Earth.pos.y += Earth.v.y * dt;
dist_x = Moon.pos.x - Earth.pos.x;
dist_y = Moon.pos.y - Earth.pos.y;
dist = sqrt(dist_x * dist_x + dist_y * dist_y); if (dist == 0) dist = 0.01;
sin = dist_x / dist; // sin
cos = dist_y / dist; // cos
Vn1 = Earth.v.x * sin + Earth.v.y * cos;
Vn2 = Moon.v.x * sin + Moon.v.y * cos;
float Vt1 = -Earth.v.x * cos + Earth.v.y * sin;
float Vt2 = -Moon.v.x * cos + Moon.v.y * sin;
...
Moon.v.x = Vn2 * sin - Vt2 * cos;
Moon.v.y = Vn2 * cos + Vt2 * sin;
Earth.v.x = Vn1 * sin - Vt1 * cos;
Earth.v.y = Vn1 * cos + Vt1 * sin;
Moon.pos.x += Moon.v.x * dt;
Moon.pos.y += Moon.v.y * dt;
Earth.pos.x += Earth.v.x * dt;
Earth.pos.y += Earth.v.y * dt;
...
//shape.setRadius(Earth.r / pow(10, 5));
//shape.setOrigin(Earth.r / pow(10, 5), Earth.r / pow(10, 5));
//shape.setPosition(Earth.pos.x, Earth.pos.y);

//shape.setFillColor(colEarth);
